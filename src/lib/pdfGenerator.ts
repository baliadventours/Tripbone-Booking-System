...
export async function generateTourVoucherPdf(booking: any, config: any) {
  ...
  drawInfoBox('LOCATION / HOTEL NAME', booking.customerData.pickupAddress || 'Please contact for pickup location', margin + 5, currentY + 8, contentWidth - 10);
  ...
  // QR in bottom right
  try {
    const appUrl = typeof window !== 'undefined' ? window.location.origin : (process.env.VITE_APP_URL || 'https://baliadventours.com');
