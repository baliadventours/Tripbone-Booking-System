    ...
    <div className="min-h-screen bg-white w-full">
      ...
      {isMobile ? (
        ...
        <div className="fixed bottom-0 left-0 right-0 z-[70] bg-white border-t border-gray-100 p-4 flex items-center justify-between gap-4 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] md:hidden">
          ...
          <Link 
            to={`/checkout/${tour.id}`}
            className="flex-1 bg-primary text-white py-4 rounded-[50px] font-black text-sm text-center shadow-lg shadow-emerald-100 active:scale-95 transition-all"
          >
            Book Now
          </Link>
        </div>
        ...
        <div className="px-6 py-10 space-y-12 pb-32">
              ...
              {(tour.itinerary || []).map((step, i) => (
                <div key={i} className="relative group">
                  {i !== (tour.itinerary || []).length - 1 && (
                    <div className="absolute left-[15px] top-8 bottom-[-40px] w-0.5 bg-emerald-100/50" />
                  )}
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 space-y-3">
                      <div className="flex flex-col gap-1">
                        ...
                        {step.pickup && (
                          <div className="flex items-center gap-1.5 text-primary font-bold text-[9px] bg-emerald-50 w-fit px-2 py-0.5 rounded-full border border-emerald-100">
          ...
          <section className="space-y-4">
            ...
            <div className="bg-gray-50 border border-gray-100/65 rounded-xl p-4.5 space-y-6">
                    ...
                    <ul className="space-y-1.5">
                      {(section.content || []).filter((line: string) => line.trim() !== '').map((point: string, pIdx: number) => (
                        <li key={pIdx} className="flex items-start gap-2">
                          ...
                          <span className="text-sm md:text-base text-gray-600 leading-snug">{point}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="space-y-6 pt-5 border-t border-gray-200">
                {/* General Info Fallback */}
                {tour.importantInfo && (
                  <div className="space-y-1.5">
                    <h3 className="text-sm md:text-base font-bold text-gray-800">Policy & Terms</h3>
                    <p className="text-sm md:text-base leading-snug text-gray-600 whitespace-pre-wrap">
                      {tour.importantInfo}
                    </p>
                  </div>
                )}
                ...
                {tour.languages && tour.languages.length > 0 && (
                  <div className="space-y-1.5">
                    <h3 className="text-sm md:text-base font-bold text-gray-800">Languages Offered</h3>
                    <div className="flex flex-wrap gap-1.5">
                      {tour.languages.filter(l => l.trim() !== '').map((lang, idx) => (
                        <span key={idx} className="px-3.5 py-1.5 bg-white border border-gray-100 font-bold text-gray-700 rounded-full text-xs md:text-sm shadow-sm">
                          {lang}
                        </span>
                      ))}
                    </div>
          ...
          {similarTours.length > 0 && (
            <section className="space-y-8 pt-8 border-t border-gray-100">
               <div className="flex items-center justify-between">
                  ...
                  <Link to="/tours" className="text-[10px] font-black text-primary uppercase tracking-widest bg-emerald-50 px-3 py-1.5 rounded-full">Explore All</Link>
               </div>
          ...
          {similarTours.length > 0 && (
            <div className="mt-24 pt-16 border-t border-gray-100">
              <div className="flex items-end justify-between mb-12">
                ...
                <Link to="/tours" className="group flex items-center gap-2 text-sm font-black text-primary uppercase tracking-widest h-12 px-8 rounded-full border-2 border-emerald-50 hover:bg-emerald-50 transition-all">
