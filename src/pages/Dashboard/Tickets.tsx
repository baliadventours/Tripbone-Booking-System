...
export default function Tickets() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'replied':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-xs font-bold uppercase tracking-wider">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
            <LifeBuoy className="text-emerald-500 h-7 w-7" /> Support Tickets
          </h1>
          <p className="text-sm text-gray-400 mt-1">Need help? Create a support request or contact Bali Adventours team.</p>
        </div>
        <button
          onClick={() => setIsCreateOpen(true)}
          className="inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-bold text-white bg-emerald-500 hover:bg-emerald-600 rounded-[12px] shadow-md shadow-emerald-500/10 transition-all cursor-pointer"
        >
              ...
              tickets.map((t) => (
                <button
                  ...
                  className={cn(
                    ...
                    t.id === activeTicketId ? "border-l-emerald-500 bg-emerald-50/10" : "border-l-transparent"
                  )}
                >
        ...
        <div className={cn(
          ...
          {activeTicket ? (
                ...
                {activeTicket.status === 'closed' ? (
                  ...
                  <form onSubmit={handleSendReply} className="flex gap-3">
                    <input
                      ...
                      className="flex-1 py-3 px-4 bg-gray-50 focus:bg-white border border-emerald-50 rounded-[12px] focus:outline-none focus:border-emerald-500 text-sm font-medium transition-all"
                    />
                    <button
                      type="submit"
                      disabled={!replyText.trim()}
                      className="p-3 bg-emerald-500 hover:bg-emerald-600 disabled:bg-gray-100 disabled:text-gray-400 text-white rounded-[12px] transition-colors cursor-pointer shrink-0"
                    >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-gray-400 h-[600px]">
              <LifeBuoy className="h-16 w-16 text-emerald-100 mb-4 animate-pulse" />
            ...
            >
              <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                <h2 className="text-md font-black text-gray-900 uppercase tracking-widest flex items-center gap-2">
                  <LifeBuoy className="h-5 w-5 text-emerald-500" /> Create New Support Ticket
                </h2>
                <button
              ...
              <form onSubmit={handleCreateTicket} className="p-6 space-y-5">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full font-semibold text-sm border-2 border-emerald-50 rounded-xl px-4 py-3 bg-emerald-50/10 focus:bg-white focus:border-emerald-500 focus:outline-none transition-all"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full font-bold text-sm border-2 border-emerald-50 rounded-xl px-4 py-3 bg-emerald-50/10 focus:bg-white focus:border-emerald-500 focus:outline-none transition-all"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    className="w-full font-semibold text-sm border-2 border-emerald-50 rounded-xl px-4 py-3 bg-emerald-50/10 focus:bg-white focus:border-emerald-500 focus:outline-none transition-all resize-none"
                  />
                </div>
                ...
                <div className="flex gap-4 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="flex-1 py-3 bg-emerald-500 hover:bg-emerald-600 disabled:bg-gray-200 text-white rounded-xl text-sm font-black uppercase tracking-wider transition-colors cursor-pointer"
                  >
