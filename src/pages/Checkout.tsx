...
import { Tour, TourPackage, Booking, AddOn, Coupon, UserProfile } from "../types";
import {
  ChevronRight,
  ChevronDown,
  Check,
...
export default function Checkout() {
  ...
  const summary = useMemo(() => {
    if (!selectedPackage)
      return { packageTotal: 0, addonsTotal: 0, discount: 0, grandTotal: 0, amountToPay: 0 };
    const packageTotal = calculatePackagePrice(selectedPackage);
    ...
    const grandTotal = Math.max(0, packageTotal + addonsTotal - discount);
  ...
  }, [selectedPackage, selectedAddOns, adults, children, appliedCoupon, existingBooking, userProfile]);
...
const toggleAddOn = (addon: AddOn) => {
  ...
  const handleFinalBooking = async (paymentId?: string) => {
    ...
    try {
      ...
      try {
        const docRef = await addDoc(collection(db, "bookings"), { ...bookingData, createdAt: serverTimestamp() });
                ...
                <div key={s.id} className="flex items-center gap-1.5 md:gap-2">
                  <div
                    className={cn(
                      ...
                      isCurrent ? "bg-primary text-white ring-4 ring-emerald-50" : 
                  ...
                  <span
                    className={cn(
                      ...
                      isCurrent ? "text-primary" : isPast ? "text-emerald-500" : "text-gray-400",
                    )}
                  >
                    <span className="hidden xs:inline">{s.label}</span>
                  </span>
                  {i < 2 && <div className={cn("h-[1px] w-4 md:w-6 bg-gray-100", isPast && "bg-emerald-200")} />}
                </div>
              );
            })}
          </div>
              ...
              <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4">
                  ...
                  <div className="bg-white rounded-[10px] border border-slate-200 shadow-md overflow-hidden">
                    <div className="p-5 space-y-5">
                      ...
                      <button 
                        ...
                        className={cn(
                          ...
                          date ? "border-emerald-600 ring-1 ring-emerald-100 shadow-sm" : "border-slate-300 hover:border-slate-400"
                        )}
                      >
                        <div className="flex items-center gap-3">
                          <div className={cn(
                            "h-9 w-9 rounded-lg flex items-center justify-center transition-colors",
                            date ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-500"
                          )}>
                                        ...
                                        className={cn(
                                          ...
                                          date === dateStr 
                                            ...
                                            : isPast 
                                              ...
                                              : "text-slate-900 bg-slate-50 border border-slate-100 hover:border-emerald-600 hover:bg-emerald-50"
                                        )}
                                      >
                                        {d}
                                      </button>
                      ...
                      <div id="traveler-selection-mobile" className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Adults */}
                        <div className="flex items-center justify-between p-4 rounded-xl border-2 border-slate-300 bg-slate-50/50 hover:bg-white hover:border-emerald-600 transition-all shadow-sm">
                          <div>
                    ...
                    {date && spotsLeft !== null && (
                      <div className={cn(
                        ...
                        isSoldOut ? "bg-rose-50 text-rose-600" : isLowCapacity ? "bg-amber-50 text-amber-700" : "bg-emerald-50 text-emerald-800"
                      )}>
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            isSelected
                              ? "border-emerald-600 shadow-md shadow-emerald-700/5"
                                    ...
                                    <div className="mt-2 text-xs font-semibold text-slate-655 leading-relaxed text-left">
                                      ...
                                      {pkg.details.length > 100 && (
                                        <button
                                          ...
                                          className="text-emerald-600 hover:text-emerald-700 font-extrabold inline-flex items-center gap-0.5 mt-1 cursor-pointer focus:outline-none block"
                                        >
                                  ...
                                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-3 text-xs text-slate-500 font-bold text-left font-sans">
                                    ...
                                    {pkg.meetingPoint && (
                                      <div className="flex items-center gap-1.5 shrink-0">
                                        ...
                                        <span className="text-slate-600">{(pkg.meetingPointType || 'Meeting Point')}: <span className="font-semibold text-slate-500">{pkg.meetingPoint}</span></span>
                                      </div>
                                    )}
                                  </div>
                                </div>
                            ...
                            {isSelected && (
                              ...
                              >
                                <div className="p-5 md:p-6 space-y-6 text-left">
                                  ...
                                  <div className="bg-emerald-500/[0.04] border border-emerald-500/10 rounded-xl p-4 space-y-2 text-left mb-6">
                                    ...
                                    <div className="space-y-2 mt-4">
                                       <div className="flex items-center justify-between text-left">
                                          <div className="flex items-center gap-1.5">
                                             <div className="h-3 w-0.5 bg-emerald-600 rounded-full" />
                                             <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Dynamic Group Rates</h4>
                                          </div>
                                          <span className="text-[9px] font-extrabold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100/60 font-mono">
                                             Active: {adults} pax
                                          </span>
                                       </div>
                                                ...
                                                return (
                                                   <div 
                                                      ...
                                                      className={cn(
                                                         ...
                                                         isActive 
                                                           ? "bg-emerald-500/[0.04] text-emerald-950 font-medium" 
                                                           : "text-slate-650 hover:bg-slate-100/20"
                                                      )}
                                                   >
                                                      <div className="flex items-center gap-2">
                                                         <span className={cn("text-[11px] font-bold", isActive ? "text-emerald-900" : "text-slate-600")}>
                                                         ...
                                                         {isActive && (
                                                            <span className="text-[8px] font-black text-emerald-800 bg-emerald-100 px-1.5 py-0.5 rounded uppercase tracking-wide">
                                                               Active
                                                            </span>
                                                         )}
                                                      </div>
                                                      ...
                                                      <div className="flex items-center gap-4 text-slate-500">
                                                         <div className="flex items-center gap-1">
                                                            ...
                                                            <span className={cn("font-bold font-mono text-[11px]", isActive ? "text-emerald-900 font-extrabold" : "text-slate-700")}>
                                                               <FormattedPrice amount={tier.adultPrice} />
                                                            </span>
                                                         </div>
                                                         <div className="flex items-center gap-1">
                                                            <span className="text-[9px] text-slate-400 font-medium font-sans">Child:</span>
                                                            <span className={cn("font-bold font-mono text-[11px]", isActive ? "text-emerald-700 font-extrabold" : "text-slate-600")}>
                                  ...
                                  <div className="grid md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
                                    {pkg.inclusions && pkg.inclusions.filter(Boolean).length > 0 && (
                                      <div className="space-y-3 text-left">
                                        <h4 className="text-xs font-bold text-emerald-700 flex items-center gap-1.5 font-extrabold uppercase tracking-wider">
                ...
                <div className="pt-8 hidden md:flex flex-col items-end gap-4">
                  ...
                  <button
                    ...
                    className="bg-primary text-white px-12 py-5 rounded-full font-black uppercase tracking-[0.2em] text-xs shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSoldOut ? 'Sold Out' : (spotsLeft !== null && (adults + children) > spotsLeft) ? 'Not Enough Spots' : 'Continue To Details'} <ChevronRight className="h-4 w-4" />
                  </button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      {customerData.phone && customerData.nationality && (
                        <p className="text-[11px] text-emerald-600 font-bold px-1 animate-in fade-in">
                ...
                <div className="hidden md:flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="bg-primary text-white px-12 py-5 rounded-full font-black tracking-[0.2em] text-xs shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all flex items-center gap-3"
                  >
                  ...
                  summary.grandTotal <= 0 ? (
                    <div className="hidden md:flex justify-end">
                      <button
                        ...
                        className="w-full sm:w-auto bg-emerald-600 text-white px-16 py-6 rounded-full font-black tracking-[0.2em] text-sm shadow-2xl shadow-emerald-100 hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isBooking ? (
                    ...
                    <div className={cn("w-full relative z-0 min-h-[150px] flex flex-col transition-opacity", !agreedToTerms && "opacity-50 pointer-events-none")}>
                      ...
                      {paymentSettings.isPaypalEnabled && (paymentSettings.paypalMode === 'live' ? paymentSettings.paypalClientId : (paymentSettings.paypalSandboxClientId || paymentSettings.paypalClientId)) && (
                        <div className="w-full">
                      ...
                      {!(paymentSettings.paypalMode === 'live' ? paymentSettings.paypalClientId : (paymentSettings.paypalSandboxClientId || paymentSettings.paypalClientId)) && (
                      ...
                      {appliedCoupon ? (
                        <div className="flex items-center justify-between bg-emerald-50 p-3 rounded-xl border border-emerald-100">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold text-emerald-900">{appliedCoupon.code} Applied</span>
                          </div>
                          <button
                            onClick={() => setAppliedCoupon(null)}
                            className="text-[10px] font-black text-emerald-600 hover:text-emerald-800"
                          >
                            Remove
                          </button>
                        </div>
        ...
        {showMobileSummary && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-6">
                <div className="flex gap-4">
                  <div className="h-16 w-16 rounded-xl bg-emerald-50 flex items-center justify-center shrink-0">
          ...
          {step === "selection" ? (
            <button
              ...
              className={cn(
                ...
                (date && selectedPackage && (adults + children > 0)) 
                  ? "bg-primary text-white shadow-emerald-100" 
            ...
            <button
              ...
              className="bg-emerald-600 text-white px-8 py-4 rounded-full font-black uppercase tracking-widest text-[10px] shadow-lg shadow-emerald-100 hover:bg-emerald-700 disabled:opacity-50"
            >
