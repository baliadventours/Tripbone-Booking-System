...
export async function generateManifestPdf(booking: any, config: any): Promise<Buffer> {
  ...
  drawInfoBox('PICKUP ADDRESS / HOTEL NAME', booking.customerData?.pickupAddress || 'Meet at location', margin + 5, currentY + 4, colWidth + 20);
  ...
  // QR in bottom right
  try {
    const appUrl = (process.env.VITE_APP_URL || 'https://baliadventours.com').replace(/\/$/, '');
