...
import { getAdminApp, getAdminDb } from "./firebaseAdmin.js";
...
export async function handleSendEmail(reqBody: any, authHeader?: string) {
    ...
    let isAdmin = decodedToken && (userEmail === rawAdminEmail || isRoleAdmin);
    ...
    if (decodedToken) {
      try {
        ...
        console.warn("[Email Handler] Firestore user role lookup skipped or failed:", err);
      }
    }
    ...
    if (!type) {
      ...
      if (!isAdmin) {
        throw new Error("Unauthorized: Only admins can send test emails.");
      }
    } else if (booking) {
    ...
    try {
      ...
      if (type) {
        ...
        if (template) {
          ...
          if (booking) {
            supplierInfo = `
              <div style="margin-top: 30px; padding: 20px; border: 1px solid #e2e8f0; border-radius: 4px;">
                <div style="font-weight: 800; font-size: 11px; color: #94a3b8; margin-bottom: 12px; text-transform: uppercase; letter-spacing: 1px;">Partner Information</div>
                <table width="100%" style="font-size: 14px; border-collapse: collapse;">
                  <tr>
                    <td style="padding: 4px 0; color: #64748b; width: 100px;">Business</td>
                    <td style="padding: 4px 0; font-weight: 700; color: #0f172a;">${supplierName}</td>
                  </tr>
                  ${(supplierPhone !== 'N/A') ? `
                  <tr>
                    <td style="padding: 4px 0; color: #64748b;">Phone</td>
                    <td style="padding: 4px 0; font-weight: 700; color: #0f172a;">${supplierPhone}</td>
                  </tr>
                  ` : ''}
                  ${(supplierEmail !== 'N/A') ? `
                  <tr>
                    <td style="padding: 4px 0; color: #64748b;">Email</td>
                    <td style="padding: 4px 0; font-weight: 700; color: #0f172a;">${supplierEmail}</td>
                  </tr>
                  ` : ''}
                </table>
              </div>
          ...
          const placeholders: any = {
              ...
              <div style="margin-bottom: 40px; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                  <div style="background-color: #f8fafc; padding: 15px 20px; border-bottom: 1px solid #e2e8f0;">
                      <div style="font-size: 13px; font-weight: 800; color: #1a1a1a; display: flex; align-items: center;">
                          <span style="font-size: 16px; margin-right: 8px;">💳</span> Payment Instructions
                      </div>
                  </div>
            ...
            "{{appUrl}}": process.env.VITE_APP_URL || "baliadventours.com",
          ...
          if (isVisualTemplate) {
            ...
            if (type.includes('confirmed') || type.includes('confirm')) {
            ...
            const showImportantNote = type === 'booking_confirmed' || type === 'booking_status_confirmed' || type === 'booking_change_approved' || type.includes('confirm');
            const showVoucher = type === 'booking_confirmed' || type === 'booking_status_confirmed' || type === 'booking_change_approved' || type.includes('confirm');
