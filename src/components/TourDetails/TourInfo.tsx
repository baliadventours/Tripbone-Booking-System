    ...
    <div className="space-y-12">
        ...
        <ul className="grid gap-y-2.5 gap-x-6 sm:grid-cols-2">
          {(tour.highlights || []).filter(line => line.trim() !== '').map((item, idx) => (
            <li key={idx} className="flex items-start gap-2.5">
              <div className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-emerald-100 text-primary mt-0.5 animate-fade-in">
      ...
      <section id="info" className="scroll-mt-[116px]">
        ...
        <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden group">
          ...
          <div className="absolute bottom-0 left-0 -mb-10 -ml-10 h-40 w-40 bg-emerald-500/10 rounded-full blur-3xl" />
                  ...
                  <ul className="space-y-2">
                    {(section.content || []).filter(line => line.trim() !== '').map((point, pIdx) => (
                      <li key={pIdx} className="flex items-start gap-3 group/item">
                        <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white/10 text-primary group-hover/item:bg-primary group-hover/item:text-white transition-all mt-0.5">
                          <Check className="h-3 w-3" />
                        </div>
                        <span className="text-sm md:text-base text-gray-300 leading-normal font-medium group-hover/item:text-white transition-colors">
                          {point}
                        </span>
                      </li>
                    ))}
            ...
            <div className="space-y-8 pt-6 border-t border-white/10">
              ...
              {tour.importantInfo && (
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <div className="h-1 w-6 bg-primary rounded-full transition-all" />
                    <h3 className="text-lg font-black text-white uppercase tracking-wider">Policy & Terms</h3>
                  </div>
                  <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                    <p className="text-sm md:text-base leading-normal text-gray-400 font-medium whitespace-pre-wrap">
                      {tour.importantInfo}
                    </p>
                  </div>
                </div>
      ...
      <section className="rounded-[10px] bg-primary p-8 text-white">
        <div className="flex flex-col items-center justify-between gap-6 md:flex-row">
          ...
          <button 
            ...
            className="flex items-center gap-2 rounded-[10px] bg-white px-8 py-4 font-bold text-primary transition-all hover:bg-emerald-50 active:scale-95 shadow-lg"
          >
